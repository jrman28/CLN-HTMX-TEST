...
def htmx_getinfo(plugin):
    ...
    html_content = f"<table><tr><th>Field</th><th>Value</th></tr><tr><td>Node ID:</td><td>{node_id}</td><td>Alias</td><td>{node_alias}</td><td>{node_id}</td>Color<td>{node_color}</td><td>{node_id}</td>Block Height<td>{block_height}</td></tr></table>"
